...
[ロードマップ](docs/03-roadmap.md) の **Step 5（繰り返し予定）まで実装済み**。
次は Step 6（CSV取込と一括削除）。
...
DB スキーマと RLS は `supabase/tests/run.sh` で検証済み（68項目）。
日付の計算・絞り込み・繰り返しの展開は `pnpm test` で検証済み（81項目）。
