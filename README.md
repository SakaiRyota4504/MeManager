...
| 機能 | 概要 | 状態 |
| --- | --- | --- |
| スケジュール管理 | 家族の予定を共有カレンダーで管理する | 主要機能は実装済み（第1弾） |
| 家計簿 | 収支の記録・集計・予算管理 | **要件定義中（第2弾）** |
...
| [docs/07-budget-requirements.md](docs/07-budget-requirements.md) | 家計簿の要件定義（第2弾・着手中） |
...
[ロードマップ](docs/03-roadmap.md) の **Step 7（取り込みの繰り返しを楽にする）まで実装済み**。
次は Step 8（デスクトップアプリ）。
...
- CSV と .ics の取り込み（プレビュー付き）と、取り込み1回ぶんの一括削除・復元
- 文字コードの自動判定、列の指定、取り込み設定の保存

カレンダーは画面の端まで使う。既定は月表示。
予定の入力は開始・終了を常に同じ位置に出し、押した日付が初期値に入る。

DB スキーマと RLS は `supabase/tests/run.sh` で検証済み（85項目）。
日付・絞り込み・繰り返し・取り込みの読み取りは `pnpm test` で検証済み（130項目）。
