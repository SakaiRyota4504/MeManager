...
メンバーの行の左にある**丸を押す**と、色を選べる。
自分の色は自分で、管理者は全員の色を変えられる。

**カレンダーの予定の色は、その予定の最初の担当者の色**で決まる。
家族ごとに重ならないよう自動で割り当てているので、ふだんは触らなくてよい。
...
やることは初回と同じで、**[`supabase/schema.sql`](../supabase/schema.sql) を
Raw からコピーして、SQL Editor で実行する**だけ。

> **何度流しても同じ結果になる。** すでに入っているものは飛ばすので、
> 「どこまで実行したか」を覚えておく必要はない。入っているデータも消えない。
> 迷ったら、いつでも schema.sql を流し直してよい。

`supabase/migrations/` の中身を1本ずつ実行する必要は無い。
あれは履歴として置いてあるもので、貼り付けて使うのは schema.sql のほう。
...
### `relation "families" already exists`

**古い schema.sql を使っている。** 2026年9月22日より前のものは、
すでにテーブルがあるデータベースに流すと、ここで止まっていた
（1つでも失敗すると全部が取り消されるので、**何も入らない**）。

いまの schema.sql は何度でも流せる。Raw から取り直して、もう一度実行する。
