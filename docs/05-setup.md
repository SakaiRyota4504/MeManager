...
| | どこで作る | 向いている場面 |
| --- | --- | --- |
| **A** | アプリの**メンバー**画面 | ふだんはこちら。Supabase を開かなくてよい |
...
1. アプリの **メンバー** 画面で表示名とメールアドレスを登録する
   （**パスワードは空のまま**「追加」を押す）
...
| ログインする | アプリのURL → ログイン |
| 表示名・家族の名前を直す | アプリのURL → メンバー → 名前を変える |
...
先にアプリの **メンバー** 画面でアドレスを登録してから作り直す（3-2 B）。
