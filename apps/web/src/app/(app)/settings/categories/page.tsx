...
import { fetchCategoryStatus, fetchTotalBudget } from "@/lib/budget/queries";
...
export default async function CategoriesPage() {
  ...
  const [categories, totalBudget] = await Promise.all([
      ...
      <section className="space-y-3">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted">
            {`予算は${formatMonth(month)}のぶんです。空欄にすると「決めていない」になり、残りは出しません。決め直すまで、翌月からも同じ額が続きます。`}
          </p>
        </div>

        <TotalBudget month={month} amount={totalBudget} />
